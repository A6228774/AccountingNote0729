...
namespace tryWebAPI0811
{
    public class WeatherDataReader
    {
        ...
        {
            string url = "https://opendata.cwb.gov.tw/fileapi/v1/opendataapi/F-B0053-043?Authorization=CWB-261FD690-AAE0-478A-969B-B2E1D5ACD451&downloadType=WEB&format=JSON";
                ...
                if (string.Compare("太魯閣國家公園太魯閣遊客中心", item.locationName, true) == 0)
                {
                    foreach (var weatherItem in item.weatherElement)
                    {
                        if (weatherItem.elementName == "T")
                        {
                            ...
                            MeasureObject measure = Newtonsoft.Json.JsonConvert.DeserializeObject<MeasureObject>(jsontxt);
